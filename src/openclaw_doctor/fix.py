...
def restart_gateway():
    ...
    cmd = f"{OPENCLAW_BIN} daemon restart 2>&1"
    ...
    if code == -1 and output == "Command timeout":
        ...
        status_cmd = f"{OPENCLAW_BIN} gateway probe 2>&1"
        status_code, status_out, _ = run_command(status_cmd, timeout=10)
        if status_code == 0 and "Reachable: yes" in status_out:
            print("✅ Gateway 已在运行")
